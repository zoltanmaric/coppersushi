...
What our model does not copy from the TSOs: which elements they choose to monitor (in Core, those where a zone-to-zone trade moves at least 5 % of its power, per the TSOs' [methodology note](literature/core-da-ccm-explanatory-note.md)), how they assume a zone's extra export is spread over its plants, tomorrow's outages and phase-shifter settings in their base case, the exact safety margins, and the extra limits they put on whole zones for stability. The [congestion-forecast spec](specs/core-congestion-forecast.md) therefore hands the model the market's rulebook, constraints only on the monitored elements at the market's margins and every other line free, and scores its binding set against JAO's. Where it misses, the reason is on that list.

...
core-capacity-calculation restructured and renamed core-day-ahead-capacity-calculation: the reference for the process the forecast emulates, with the project's stand-ins moved to the spec. Vocabulary first; then one row, Elia's Lixhe–Gramme under a tripod outage, read column by column on 2024-08-28 and 29, showing what persists overnight (element, contingency, margin share, factor) and what moves (flows, PTDFs by hundredths, this time the seasonal rating); then the eleven steps. Verified on JAO's rows: `ram = fmax − frm − (fcore + fltn) + amr − iva` on all 116 presolved rows of the first hour; the minimum-RAM lift in its two-floor form, `minRamTarget = max(0.20, minRamFactor / 100 − fuaf / fmax)`, on all 11,560 element rows; `fref = frefInit − fnrao` on all 7,019 rows with a remedial-action effect, the column being a reduction; `fmax = √3 · 400 kV · imax` on all 10,080 rows of the 380 kV level; `fcore = fref − Σ ptdf · NP` at D2CF's net positions with ALEGrO at its 713 MW reference flow, within 15 MW on all element rows. Day to day: 479 of 487 elements, 97 % of rows and 89 of 108 presolved element rows shared between the two days. A goldfish found what the first draft missed: the 116 presolved rows include four equality rows, the hub balances, beside ALEGrO's four bounds; `ltaMargin` is zero everywhere because EUPHEMIA takes the union of the rows and the long-term domain itself, so a binding long-term facet has no shadow-price row; `frm` is the flat 10 % default on every row; Poland's net position was capped every hour through the allocation-constraint feed; Coreso is the merging agent. A section reads the three rows that bound at the same hour, room used to the megawatt and the shadow price of one more, against the slack of the other 106. New wiki rule `page-names`, after two generic names in one day.
...
## [2026-09-09] decision | Core congestion forecast: rebuild the rows, clear a zonal market, look
One day, 2024-08-29, as a hindcast; the day is a parameter. The auction sees the grid only through about 120 rows an hour, so the forecast rebuilds D-1's rows for D with our grid's flows on the matched elements and clears cost-based zonal bids against them in a small LP; net positions, binding rows, shadow prices and implied spreads fall out. Three runs of the same LP, D's published rows, D-1's rows unchanged, D-1's rows with our flows, separate the bid proxy from the grid. Deliverable: a page with JAO's binding elements beside ours and a written explanation; scoring, baselines and more days deferred until one day has been read by eye. Rejected in the grill: a nodal solve with constraints on the monitored elements (the auction moves net positions through the shift key, never plants against a line, so it solves a different problem); pinning load to D2CF's vertical load (net of embedded generation, a tenth of consumption in Germany at noon); zone caps as a class (the day's four were ALEGrO's); our own PTDFs through a shift key for now (D-1's move by hundredths overnight). A goldfish on the rewrite found our base case's exchanges with the outside about 2 GW per zone off JAO's, Ukraine and Moldova exporting into Core and France into Switzerland against RefProg, so the LP's balances take RefProg; and that the domain's four equality rows, not the external constraints, carry the hub balances. The base case for run 3 is a copper-plate dispatch at run 2's net positions with a plain load flow, not the capped OPF's solution, whose dispatch dodges the congestion the market cannot see; that retires the 2 GW exchange errors and the fixed ALEGrO flow as artefacts of that solution. Cut the same day to what needs no matching: clear cost bids against the previous day's rows, draw the copper-plate load flow of that trade; rebuilding the rows with our flows is later work. Reservoir hydro bids a water value, run-of-river runs at inflow, pumped storage waits.

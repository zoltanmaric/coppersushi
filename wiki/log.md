...
## [2026-09-09] query | The day-ahead chain step by step: what is published, what we substitute
core-capacity-calculation restructured and renamed core-day-ahead-capacity-calculation. Vocabulary first, then the eleven steps as prose, each with its inputs, what JAO publishes and when, and the stand-in available at D-2 evening; then the shortest faithful approximation (D-1's rows as skeleton, our grid for the day's flows, RAM by the identity, a zonal clearing); the actor table last. Two identities verified on 2024-08-29's first hour: `ram = fmax − frm − (fcore + fltn) + amr − iva` on all 116 presolved rows, and the minimum-RAM lift against the flow without any Continental exchange on all 459 lifted rows. The closing section states the auction's degrees of freedom: net positions move and the shift key spreads them, so a model that redispatches inside a zone solves a different problem. New wiki rule `page-names`, after two generic names in one day.

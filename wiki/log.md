...
## [2026-09-08] decision | Core congestion forecast as the hackathon cut
Replaces the price-anatomy plan. Electricity Maps' 72-hour zonal forecasts (mix, load, exchanges) are pinned into the nodal OPF; lines at their limit are the predicted binding elements and nodal price gaps the predicted zonal splits, scored daily against JAO's public shadow prices and settled prices. Chosen because it is the one analysis with a market behind it; the TSOs' full flow-based procedure is explicitly out of scope. Spec in specs/core-congestion-forecast; graph gains four planned nodes.

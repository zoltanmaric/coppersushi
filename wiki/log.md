...
The fork's `master` stays a mirror of upstream so each rehearsal PR shows one fix; the pin points at the `coppersushi` branch — the fix branches rebased onto upstream and nothing else, synced by rebase. Rebasing orphans old pinned SHAs, so every pin gets an immutable `pin/<YYYY-MM-DD>-<what is new>` tag on the fork; a rule, not a check in the runner. Rejected: fixes on the fork's `master` (breaks the rehearsal PRs' base and wins nothing, the runner is branch-blind) and merge-sync (keeps SHAs but buries the delta in merge commits).

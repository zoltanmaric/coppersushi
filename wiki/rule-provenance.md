...
| `mermaid` no init theme; fill only with colour | Two spec diagrams with light fills and no text colour rendered grey-on-white in GitHub's dark theme; the user had hit the same on another project. Aligned with Princeton University Library's accessible-Mermaid guide and GitHub community discussion 35733 |
| `mermaid` no `<x>` placeholders in labels | `&lt;day&gt;` in two spec diagrams rendered as nothing: "networks/opf-.nc" |
| `mermaid` shapes carry roles; `planned` the one class | Adopted with the skill from the architecture graph's convention; no separate occurrence |

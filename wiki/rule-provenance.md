...
| `term-anchors` (wiki) | Terms defined in a bullet list on one page and met cold on another: on 2026-09-09 the user could not place D2CF in the forecast spec after the concept page had defined it, hours after that page itself had to be rewritten because its terms had no definitions a reader could reach; the first link form, GitHub's slug of a two-word heading, then failed to resolve in Obsidian. The same failure then appeared on the ordinary heading `What it settles`, while a lint found it twice more on `Fork refs (remote fork)`, proving the mechanism must cover every cross-page heading target. |

...
- **`pypsa-eur.pin`**: two lines, a repository URL and a commit SHA; the runner fetches that commit from that URL into the sibling. It is a tested head of the fork's `coppersushi` branch (Fork refs below); as of 2026-09-08 that head carries our three `clusters: all` fixes ([ledger](upstream-contributions.md)). Bumping = pointing the local pin at a newer head, solving, then committing the SHA and tagging it (`pin/*` row below).

...
| `coppersushi` | The integration branch: upstream `master` plus, cherry-picked on top, the not-yet-upstreamed fixes the solve needs — nothing else. The pin is one of its tested heads; between a rebuild and the next bump the branch runs ahead of the pin. The layers and their order are the branch's own commits: `git log --reverse --format=%s origin/master..coppersushi`. Upstream moved: `git rebase origin/master coppersushi`. A topic branch changed or a fix joins: `git switch -C coppersushi origin/master`, then `git cherry-pick origin/master..<topic>` per topic in that order, new ones last. Conflicts: `doc/release_notes.md` at every layer (keep every bullet); a test file two layers both add (keep both tests); otherwise the later layer's version. Then `git push --force-with-lease fork coppersushi`. A layer drops out when its fix lands upstream. |

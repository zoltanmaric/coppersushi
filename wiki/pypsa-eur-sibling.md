...
| `coppersushi` | What the pin points at: upstream `master` plus, cherry-picked on top, the topic branches' fixes that have not yet merged upstream — nothing else (the copies' SHAs differ from the topic branches' commits). Rebuild whenever upstream or a topic branch moves: `git switch -C coppersushi origin/master`, then `git cherry-pick origin/master..<topic>` for `fix/clustered-regions-country`, `fix/offshore-shoreline-proxy`, `fix/disaggregated-generator-names` in that order; expect conflicts in `doc/release_notes.md` (keep every line) and, at the shoreline layer, `scripts/build_renewable_profiles.py` (take the shoreline version — it subsumes the country layer's `.geometry`); then `git push --force-with-lease fork coppersushi`. A layer drops out when its fix lands upstream. |
| `pin/<YYYY-MM-DD>-<what is new>` (tags) | One per SHA ever written to `pypsa-eur.pin`, dated when pinned (`pin/2026-09-08-clusters-all-fixes`), pushed to `fork`, never moved or deleted — so a rebase never orphans a pinned commit. Tag when you pin. |

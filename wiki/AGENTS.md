...
2. **The agent owns this folder.** Wiki pages are written and maintained by the agent following the `llm-wiki` skill — keep `index.md` (content catalog) and `log.md` (append-only chronology) current with every change.

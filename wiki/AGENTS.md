...
- **term-anchors** — Define a domain term once, under its own `###` heading on the page that owns it, and link that heading from every other page at the term's first use, expansion in the link text. The heading is one token, the domain's acronym or identifier where it has one (`D2CF`, `GSK`), hyphenated words otherwise (`Net-position`), and the link uses it in lowercase, `[D2CF, the D-2 congestion forecast](core-day-ahead-capacity-calculation.md#d2cf)`: GitHub resolves a heading's lowercase slug, Obsidian its text ignoring case, and a one-token heading is the only form both accept. A gloss repeated on a second page is a copy that rots.

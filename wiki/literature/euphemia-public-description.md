...
**Two network models, both zonal.** ATC borders are pipes with capacity per direction, optional losses, tariffs and ramping limits. Flow-based regions are the linear constraint PTDF · net positions ≤ RAM, one row per element, one column per hub. Since version 10.5 the flow-based region can be handed the untouched domain and the long-term-allocation domain separately (extended LTA inclusion); EUPHEMIA respects their union itself; in Central Western Europe that replaced a projected 27,000 virtual constraints with under 800 real ones. External constraints cap a zone's net position. Below the zone, scheduling areas and NEMO trading hubs exist only for disaggregation; all orders in a zone clear at one price.

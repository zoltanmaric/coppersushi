...
**What our network is.** PyPSA-Eur lifts everything to one 380 kV layer before we ever see it (`simplify_network_to_380`, unconditional — no config switch). Our solved network: 4,390 buses and 7,371 lines, all `v_nom = 380`, **zero transformers**, down from `base.nc`'s 6,863 buses, 9,162 lines and 878 transformers across eight voltage levels. Two things follow, and they pull in opposite directions:

- **Capacities survive the lift.** `num_parallel = s_nom / (√3 · 380 · i_nom)` — MW ratings are preserved exactly and the voltage change is absorbed into a fractional circuit count (ours run 0.29 to 4.21). A 220 kV corridor keeps its 220 kV capacity under a 380 kV label. So JAO's fmax, a MW number, is directly comparable to our `s_nom` whatever voltage the element is at.
- **Transformers do not.** All 878 are deleted, each low-voltage bus collapsed into the high-voltage one at the same site. JAO monitors about 62 transformers and PSTs per hour; on this network there is no branch to write a limit onto. (Their ratings were never trustworthy anyway: OSM-derived `s_nom` from 503 to 23,238 MVA, median 4,425, with a flat `x = 0.1` p.u. for every one of them, against real Austrian 380/220 kV units at 555 to 607 MW.)

**Scope: one day, 2024-08-29** — the day the rest of the stack already solves. Everything below is built and accepted for that day alone. If a sweep over a longer range turns out to be needed to synthesise anything, that is a later, separate decision.
...
2. **True limits** for the matched *lines*, written into our network from JAO's own numbers. Transformers and PSTs are drawn and labelled but not written — see above.
...
flowchart LR
    ...
    match["match_elements<br/>substation pair → nearest buses → the line between them; all monitored elements on the pair summed; transformers and PSTs as points<br/>contingency names parsed and matched the same way"]
    true_up["true_up_limits<br/>s_nom := summed fmax per matched line (no transformers on the 380 kV layer); flag ratio &gt; 2 to the old rating"]
...
- **From substations to our lines.** Each matched pair gives two coordinates; the nearest buses in our network and the line joining them is the element. JAO lists parallel circuits separately ("Duernrohr 1 - Slavetice 437" and "438"); our network folds them into one line, so as a *monitored element* their limits are summed. The sum runs over *every* monitored element on the pair, across voltage levels too: a 380 kV and a 220 kV corridor between the same two substations are two CNECs for JAO and one line for us. As a *contingency*, one circuit out of k is not the corridor out: the folded line's reactance rises by k/(k−1) and the outage factor is computed for that reactance change, not for a removal. Only a contingency naming every circuit removes the line. Transformers and phase shifters sit inside one substation, have no branch on the 380 kV layer, and are drawn as points: their JAO limit, margin and shadow price are shown, nothing is written into the network. External constraints (caps on a zone's net position) have no location and are listed beside the map.
- **Security: what a limit means, and which version we build.** For a matched line the equipment limit becomes JAO's fmax (all monitored elements on the pair summed). What keeps the solve inside N-1 on top of that limit has two versions:
  ...
  - **Which we build.** `n-1` first — the accuracy gap is the whole difference between a map that looks plausible and one whose binding elements are the ones that actually bound. Its extra cost over `proxy` is one piece of work: matching the contingencies, whose names are free text ("Pasewalk - Bertikow 408"; the structured field is empty) and go through the same substation geocoding as the elements, plus the LDF computation. Estimate: a day on top of the matching that steps 2 and 3 do anyway. `proxy` stays in as the fallback if contingency names resist matching on 2024-08-29, and as the comparison the rating report is read against.
...
- **Non-goals**: modelling phase-shifter behaviour, anything outside Core, the TSOs' remedial actions after the auction, any treatment of OSM lines JAO does not monitor, and unsimplifying the network. Transformer and PST limits, and telling a 380 kV corridor from the 220 kV one beside it, are what unsimplifying would buy; that is its own piece of work and does not block anything here.

## Next steps

1. **JAO fetch**: an adapter that pulls the day from both endpoints into committed JSON; a hermetic test on a small fixture of rows.
...
3. **Element matching** and the map page for 2024-08-29.
...
- [ ] `true_up_limits` writes JAO limits onto matched lines, summing every monitored element on the substation pair including across voltage levels, writes hourly `s_max_pu` for dynamic ratings, and flags every match whose new limit differs from the old by more than a factor of two. Transformers and PSTs are reported as unwritable, with a count.
...
- What the 380 kV layer actually costs us: of the elements that bind on 2024-08-29, how many are transformers or PSTs (unwritable), and how many are lower-voltage corridors sharing a substation pair with a 380 kV one (indistinguishable). That count is the case for or against unsimplifying, and this work measures it for free.

...
- **Security: what a limit means, and which version we build.** For a matched line the equipment limit becomes JAO's fmax (circuits summed); transformers, once the unsimplified network is in use, take fmax in place of the 2000 MVA default. What keeps the solve inside N-1 on top of that limit has two versions:
...
- **Non-goals**: modelling phase-shifter behaviour, anything outside Core, the TSOs' remedial actions after the auction, and any treatment of OSM lines JAO does not monitor.

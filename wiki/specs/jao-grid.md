...
- **Security: what a limit means, and which version we build.** For a matched line the equipment limit becomes JAO's fmax (all monitored elements on the pair summed). What keeps the solve inside N-1 on top of that limit has two versions:
  ...
  - **Which we build.** `n-1` first — the accuracy gap is the whole difference between a map that looks plausible and one whose binding elements are the ones that actually bound. Its extra cost over `proxy` is one piece of work: matching the contingencies, whose names are free text ("Pasewalk - Bertikow 408"; the structured field is empty) and go through the same substation geocoding as the elements, plus the LDF computation. Estimate: a day on top of the matching that steps 3 and 4 do anyway. `proxy` stays in as the fallback if contingency names resist matching on 2024-08-29, and as the comparison the rating report is read against.
...
1. **Unsimplified network** (the precondition, and first because everything else lands on it): keep PyPSA-Eur's voltage levels and transformers, re-solve 2024-08-29, promote it to the shelf. The 878 transformers come back with guessed ratings; step 4 replaces the monitored ones.
...
- How many of the elements binding on 2024-08-29 are transformers or PSTs. The precondition is justified by their existing at all, but the count says how much the unsimplified network actually bought, and this work measures it for free.

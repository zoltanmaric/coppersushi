...
- **Non-goals**: modelling phase-shifter behaviour, anything outside Core, the TSOs' remedial actions after the auction, any treatment of OSM lines JAO does not monitor, and unsimplifying the network. What unsimplifying would buy is transformer and PST limits — nothing else: line capacity, reactance and voltage identity all survive the lift. That is its own piece of work and does not block anything here.
...
- [ ] `true_up_limits` writes JAO limits onto matched lines, summing only the elements that fold onto the same line and picking the line by voltage where a substation pair carries several, writes hourly `s_max_pu` for dynamic ratings, and flags every match whose new limit differs from the old by more than a factor of two. Transformers and PSTs are reported as unwritable, with a count.
...
- What the 380 kV layer actually costs us: of the elements that bind on 2024-08-29, how many are transformers or PSTs, which are the only class the lift makes unrepresentable. That count is the case for or against unsimplifying, and this work measures it for free.

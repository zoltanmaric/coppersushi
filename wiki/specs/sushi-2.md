...
- Non-goals for this spec: true-up to actuals (calibration, measured constraints, validation), carbon layer, contingencies, article copy, hosting. Forecasting lives in [core-congestion-forecast](core-congestion-forecast.md).

## Next steps

1. **Solve 2013-07-17 (v1's day) on the OSM grid and draw it** — **done 2026-09-02** (pin + runner + config; three `clusters: all` bugs patched on the fork's `coppersushi` branch; load shedding enabled as a diagnostic, any shedding rejected by the runner; HiGHS 213 s). Config as landed: pin file + runner + `config/coppersushi.yaml`; the app draws it alongside v1's network. Config: `clusters: all` (`simplify_network` always runs first — lifted to 380 kV, stubs removed — so this is the simplified bus set, as v1's was, not the raw 6864-bus OSM layer), `transmission_limit: v1.0` (genuinely fixed lines; `v1.01` sets every line extendable — use it, or `load_shedding`, only if `v1.0` is infeasible), HiGHS with `threads` raised from the profile's 1, `transmission_losses: 0`, `noisy_costs: false`, `dynamic_fuel_price: false` (the monthly-price reindex yields all-NaN costs for a window not starting on a month boundary — [ledgered](../upstream-contributions.md)). Temporal resolution: decide from the first solve time. If `all` will not solve within a day, drop to a few hundred clusters.
2. **The 2024 day**: same config, three things moved — `snapshots` to the day, `atlite.default_cutout` to `europe-2024-sarah3-era5` (the weather year follows the day; see [pypsa-eur-sibling](../pypsa-eur-sibling.md)), and `electricity.powerplants_filter` to the fleet commissioned by 2024 and not yet retired.

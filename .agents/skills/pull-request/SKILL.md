...
- **Insert or reorder layers**: GitHub only grows a stack at the top, so `submit` and `link` cannot retarget a stacked PR onto a new layer below it. Rewrite ancestry with `git rebase --onto`, then `gh stack unstack` (PRs and branches untouched), `gh stack init <b1> … <bn> --base <trunk>`, `gh stack submit`: it updates the bases and re-links.

...
def inputs() -> cnec_page.Day:
    ...
    return cnec_page.Day(
        ...
        buses=pd.read_csv(MAP / "buses.csv", index_col="bus"),
...
def test_layout_starts_on_the_supplied_day_and_exposes_all_controls():
    ...
    assert controls["cnec-date"].date == "2024-08-29"
    assert {"cnec-constraint", "cnec-reference-zone", "cnec-map", "cnec-interval"} <= set(
        controls
    )
...
def test_a_historical_day_has_every_hour_and_timezone_disambiguated_marks():
    ...
    assert rendered.interval_marks[0] == "00:00 CEST"
    assert rendered.interval_marks[23] == "23:00 CEST"
...
def test_binding_rows_are_sorted_by_shadow_price_and_keep_their_source_id():
    ...
    assert "100.00 €/MWh" in rendered.constraint_options[0]["label"]
    assert "base case" in rendered.constraint_options[1]["label"]
...
def test_a_selection_from_another_interval_is_cleared():
    ...
    assert rendered.constraint_options == []

...
- **pull-requests-via-skill** — Any work on pull requests — opening, readying, chopping into layers, restacking — goes through the `pull-request` skill before the first `gh` or `git push`. Its stack mechanics are `gh stack`; a hand-made chain of `gh pr create --base` is not a stack.

- **early-stack-prs** — Push and open each PR in a stack as soon as it has a coherent commit. Keep an unfinished layer draft; mark it ready only when that layer is complete and verified.
